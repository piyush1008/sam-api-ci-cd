...
const getAuthToken2=async()=>{
    const tokenurl="https://dev.css.api.hp.com/oauth/accesstoken";
    const params = {
        username: "XTCkUOWwIZeXgS8eBn9VIWsfEOPixI2C",
        password: "QWqYutHA6pCt0p6TYTQsgOiXAPg0EHn4"
    }
    ...
    try {
        ...
        if (response.status === 200) {
            ...
            console.log("|98923dfdds|", accessToken);
            return accessToken;
        } else {
...
const sendApiCommand1=async(apiPath, requestBody, request)=>{
    let response;

        try {
            let apiStartTime = Number(Date.now());
            console.log(`${apiPath} intent API REQUESTBODY: ${JSON.stringify(requestBody, null, 4)}`);
            console.log(`${apiPath} intent API STARTTIME: ${apiStartTime}`);
            console.log('Sending ', request);
            response = await axios(request);
            let apiEndTime = Number(Date.now());
            console.log(`${apiPath} intent API ENDTIME: ${apiEndTime}`);
            let apiDuration = apiEndTime - apiStartTime;
            console.log(`${apiPath} intent API DURATION: ${apiDuration}ms`);
            console.log(`${apiPath} intent API RESPONSE: ${JSON.stringify(response.data, null, 4)}`);
        } catch (e) {
            console.log(`${apiPath} intent :: API ERROR: ${JSON.stringify(e, null, 4)}`);
            console.log("|12312312lllll");
            console.log(e);
        }
        console.log("|213213vasdf-123|", response);
        return response;
}
const getCaseResult=async(requestBody)=>{
    
    const token=await getAuthToken2();
    const intentdetectionEndpoint="https://uids-uat.hpcloud.hp.com/uids/V1/inference"
    
    let apiPath =
    '/' +
    intentdetectionEndpoint
        .split('/')
        .slice(3)
        .join('/');

    console.log('|00-13|- >apipath', apiPath);

    let requestOptions = {
        host: intentdetectionEndpoint.split('/')[2],     // "po1tnlcn76.execute-api.us-west-2.amazonaws.com",   ///replace with parameter - no hardcoded URLS!
        method: 'GET',
        url: intentdetectionEndpoint,
        path: apiPath,
        headers: {
            'Authorization': 'bearer ' + token
        },
        params: {
            'utterance': requestBody
        },
        region: 'us-west-2',
    };

    return await this.sendApiCommand1(apiPath, requestBody, requestOptions);
